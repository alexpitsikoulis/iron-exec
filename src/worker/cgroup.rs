...
pub fn create_cgroup(
    ...
    let path_str = &format!("/sys/fs/cgroup/{}_{}", cmd_name, job_id.to_string());
    let root_cgroup_path = Path::new(path_str);
    if let Err(e) = create_dir_all(root_cgroup_path) {
        return Err(e);
    }

    if let Some(cpu_max) = config.cpu_max {
        if let Err(e) = std::fs::write(root_cgroup_path.join("cpu.max"), cpu_max.to_string()) {
            return Err(e);
        }
    }

    if let Some(cpu_weight) = config.cpu_weight {
        if let Err(e) = std::fs::write(root_cgroup_path.join("cpu.weight"), cpu_weight.to_string())
        {
            return Err(e);
        }
    }
    ...
    if let Some(memory_max) = config.memory_max {
        if let Err(e) = std::fs::write(root_cgroup_path.join("memory.max"), memory_max.to_string())
        {
            return Err(e);
        }
    }
    ...
    if let Some(io_max) = config.io_max {
        if let Err(e) = std::fs::write(root_cgroup_path.join("io.max"), io_max.to_string()) {
            return Err(e);
        }
    }

    if let Some(io_weight) = config.io_weight {
        if let Err(e) = std::fs::write(root_cgroup_path.join("io.weight"), io_weight.to_string()) {
            return Err(e);
        }
    }
